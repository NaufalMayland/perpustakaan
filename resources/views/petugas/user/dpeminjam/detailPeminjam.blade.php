...
@section('content')
            ...
            <div class="w-full lg:w-3/4 flex flex-col gap-4 text-sm lg:text-base">
                <div class="grid gap-4 text-sm mt-4">
                    <div class="grid">
                        ...
                        <input type="text" name="nik" id="nik" class="w-full focus:outline-none p-2 rounded border bg-gray-100 border-gray-300 text-sm" value="{{ $peminjam->id }}" readonly>
                    </div>
                    <div class="grid">
    ...
    <form action="{{ route('petugas.user.dpeminjam.ubahPasswordDpeminjam', $peminjam->id) }}" method="POST" class="bg-white rounded shadow p-4 mt-4 gap-4 text-sm grid">
        @csrf
        @if (session('errors'))
