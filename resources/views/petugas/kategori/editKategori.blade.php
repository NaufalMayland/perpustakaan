    ...
    <form method="POST" action="{{ route('petugas.kategori.editKategoriAction', $kategori->id) }}" class="bg-white p-4 shadow-md rounded" enctype="multipart/form-data">
        ...
        <div class="grid gap-2 text-sm mt-4">
            <div class="grid">
                ...
                <input type="text" name="kategori" id="kategori" class="w-full p-2 rounded border bg-gray-100 border-gray-300 text-sm" value="{{ $kategori->kategori }}" placeholder="Masukan judul buku" required>
            </div>
