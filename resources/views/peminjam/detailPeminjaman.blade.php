            ...
            <div class="bg-white p-4 rounded gap-4 flex flex-col shadow">
                <div class="grid gap-4 text-sm lg:text-base">
                    <div class="flex justify-between border-b pb-2">
                        ...
                        <span class="w-full text-gray-900 text-right">{{ $peminjaman->buku->judul }}</span>
                    </div>
                    <div class="flex justify-between border-b pb-2">
                        <span class="w-full font-semibold text-gray-700">Nama Peminjam:</span>
                        <span class="w-full text-gray-900 text-right">{{ $peminjaman->peminjam->nama }}</span>
                    </div>
                    <div class="flex justify-between border-b pb-2">
                        <span class="w-full font-semibold text-gray-700">Tanggal Pinjam:</span>
                        <span class="w-full text-gray-900 text-right">{{ \Carbon\Carbon::parse($peminjaman->tanggal_pinjam)->translatedFormat('j F Y') }}</span>
                    </div>
                    <div class="flex justify-between border-b pb-2">
                        <span class="w-full font-semibold text-gray-700">Tanggal Kembali:</span>
                        <span class="w-full text-gray-900 text-right">{{ \Carbon\Carbon::parse($peminjaman->tanggal_kembali)->translatedFormat('j F Y') }}</span>
                    </div>
                    <div class="flex justify-between border-b pb-2">
                        <span class="w-full font-semibold text-gray-700">Jumlah:</span>
                        <span class="w-full text-gray-900 text-right">{{ $peminjaman->jumlah }}</span>
                    </div>
                    @if ($peminjaman->petugas !== null) 
                        <div class="flex justify-between border-b pb-2">
                            <span class="w-full font-semibold text-gray-700">Petugas:</span>
                            <span class="w-full text-gray-900 text-right">{{ $peminjaman->petugas->nama }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="w-full font-semibold text-gray-700">Status:</span>
                        <span class="w-full text-gray-900 text-right capitalize">{{ $peminjaman->status }}</span>
                    </div>
                    @if ($peminjaman->status == "dipinjam" && $perpanjangan && $peminjaman->perpanjangan == null) 
                        <div class="flex">
                            @include('peminjam.modal.perpanjangan')
                        </div>
                    @endif
                </div>
            </div>
